...
# - 신규 유저 환영 (문구 업데이트)
# - !역할 / !국가 버튼
# - 📥보안채널📥 에서 !보안 → 비밀번호 확인(0920) 후 "보안인증서" 역할 부여
# - !회의 → R4 역할 멘션하여 회의 안내
# - !리폿 → 접수 회신 + ⚠서버-관리실⚠ 채널에 신고자 태그로 알림
# - 매일 09:00 / 매주 일 23:00 정기 안내
# - !청소, !명령어, !영이
# - 루프 중복 방지, 환경변수 체크, Interaction 처리(defer/followup)
...
# ----- 기타 -----
SECURITY_PASSWORD = os.getenv("SECURITY_PASSWORD", "0920").strip()  # 기본값 0920
...
async def cmd_security(ctx: commands.Context):
    ...
    try:
        ...
        if pw == SECURITY_PASSWORD:
            if not ROLE_SECURITY_CERT_ID:
                await ctx.send("역할 ID(ROLE_SECURITY_CERT_ID)가 설정되지 않아 역할을 부여할 수 없어요. 관리자에게 문의해주세요.")
                return
