...
async def on_interaction(inter: discord.Interaction):
    ...
    try:
        ...
        if cid == "country_kr":
            await _add_role_and_route(ROLE_LANG_KR_ID, SECURITY_CHANNEL_KR_ID, "보안 인증을 진행해주세요.", "여기서 `!보안` 을 입력하면 됩니다. (비밀번호: 0920)")
            return

        if cid == "country_en":
            await _add_role_and_route(ROLE_LANG_EN_ID, SECURITY_CHANNEL_EN_ID, "Please proceed with security verification.", "Type `!Security` here. (Password: 0920)")
            return

        if cid == "country_cn":
            await _add_role_and_route(ROLE_LANG_CN_ID, SECURITY_CHANNEL_CN_ID, "请进行安全验证。", "在此处输入 `!安全`。（密码：0920）")
            return

        if cid == "country_vi":
            await _add_role_and_route(ROLE_LANG_VI_ID, SECURITY_CHANNEL_VI_ID, "Vui lòng tiến hành xác minh bảo mật.", "Nhập `!BảoMật` tại đây. (Mật khẩu: 0920)")
            return
...
async def _security_flow(ctx: commands.Context, lang: str, need_lang_role_id: int, cert_role_id: int, allowed_channel_id: int, secondary_channel_id: int = 0):
    ...
    if allowed_channel_id and ctx.channel.id != allowed_channel_id:
        ...
        msg = {
            "KR": f"이 명령어는 {target.mention if target else '지정된 보안채널'} 에서만 사용 가능합니다.",
            "EN": f"This command can be used only in {target.mention if target else 'the Security Channel'}.",
            "CN": f"此指令仅可在 {target.mention if target else '安全通道'} 使用。",
            "VI": f"Lệnh này chỉ sử dụng tại {target.mention if target else 'kênh an ninh'}."
        }[lang]
    ...
    if need_lang_role_id:
        ...
        if need_role and need_role not in ctx.author.roles:
            msg = {
                "KR":"Korean 역할이 있어야 인증할 수 있어요. 먼저 `!국가`로 Korean을 선택하세요.",
                "EN":"You must have the English role. Run `!Country` and choose English first.",
                "CN":"需要拥有中文角色。请先运行 `!国家` 选择 China。",
                "VI":"Bạn cần có vai trò Tiếng Việt. Vào `!QuốcGia` chọn Vietnam trước."
            }[lang]
            await ctx.reply(msg); return
